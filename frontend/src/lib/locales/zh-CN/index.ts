export const zhCN = {
  ...
  connectionErrors: {
    ...
    apiDesc: "无法访问 Marinheiro de Silício API 服务器",
    dbTitle: "数据库连接失败",
    ...
    docLink: "Marinheiro de Silício 文档",
  ...
  sources: {
    ...
    confirmAddToNotebook: "添加 {count} 个到此笔记本",
    added: "已添加于 {date}",
    addUrl: "添加 URL",
    uploadFile: "上传文件",
    enterText: "输入文本",
  ...
  chat: {
    ...
    messageSources: "使用的来源",
    deleteMessageDesc: "确定要删除此消息吗？删除您的消息也会删除助手的回复。此操作无法撤销。",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "配置用于 Marinheiro de Silício 不同用途的默认模型",
    missingRequiredModels: "缺少必需的模型：{models}。如果没有这些模型，Marinheiro de Silício 可能无法正常运行。",
    ...
    visionModelDesc: "用于图像和视频描述 - 推荐 GPT-4o、Claude 或 Gemini",
  ...
  apiKeys: {
    ...
    description: "将 API 密钥安全地存储在数据库中，以在 Marinheiro de Silício 中启用 AI 服务商。",
