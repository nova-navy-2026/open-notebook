export const jaJP = {
  ...
  connectionErrors: {
    ...
    apiDesc: "Marinheiro de Silício APIサーバーに到達できませんでした",
    ...
    docLink: "Marinheiro de Silícioドキュメント",
  ...
  sources: {
    ...
    confirmAddToNotebook: "{count}件をこのノートブックに追加",
    added: "{date}に追加",
    addUrl: "URLを追加",
  ...
  chat: {
    ...
    messageSources: "使用したソース",
    deleteMessageDesc: "このメッセージを削除しますか？あなたのメッセージを削除すると、アシスタントの返信も削除されます。この操作は元に戻せません。",
  ...
  settings: {
    ...
    filesHelp: "ファイルがアップロードされて処理されると、ファイル自体は不要になります。ほとんどのユーザーはMarinheiro de Silícioがアップロードフォルダから自動的にファイルを削除することを許可すべきです。",
  ...
  advanced: {
    ...
    updateAvailableDesc: "Marinheiro de Silícioの新しいバージョンが利用可能です。",
    upToDate: "最新版です",
    unknown: "不明",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "Marinheiro de Silício全体で異なる目的に使用するモデルを設定",
    missingRequiredModels: "必須モデルがありません: {models}。これらがないとMarinheiro de Silícioが正しく機能しない可能性があります。",
    ...
    visionModelDesc: "画像や動画のキャプション生成に使用 - GPT-4o、Claude、Gemini推奨",
  ...
  apiKeys: {
    ...
    description: "APIキーをデータベースに安全に保存し、Marinheiro de SilícioでAIプロバイダーを有効にします。",
